...
axios.get('http://bestshop1.pythonanywhere.com/api/purchas/product')
.then((data)=>{
    if(data.status === 200){
...
item.addEventListener('click',(e)=>{
    ...
    axios.get(`http://bestshop1.pythonanywhere.com/api/cell/product/remove/${number}`)
    .then((data)=>{
...
busd.addEventListener('click',()=>{
    ...
    axios.put('http://bestshop1.pythonanywhere.com/api/purchas/product',JSON.stringify(data),{headers:{
      ...
      .then((data)=>{
        ...
        speechSynthesis.speak(uttrens)
            setTimeout(()=>{
                window.location.href = `http://bestshop1.pythonanywhere.com/shopping/history/pdf/${data.data.id}`
            },2500)
        }
      })
})
