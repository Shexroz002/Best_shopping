...
chatboxToggle.addEventListener('click', function () {
	...
	if(is_superuser === 'false'){
		...
		window.location.href="http://127.0.0.1:8000/users/chat"
	}
...
chatboxForm.addEventListener('submit', function (e) {
	...
	if(isValid(textarea.value)) {
		axios.post('http://127.0.0.1:8000/users/api/chat/6',JSON.stringify(data),{headers:{
		'Content-Type': 'application/json',
		'X-CSRFToken': csrftoken,}
		})
		.then(data=>{
...
axios.get('http://127.0.0.1:8000/users/api/chat/6')
.then(data=>{
  if(data.status === 200){
