...
axios.get(`http://bestshopnew.pythonanywhere.com/api/product/all`)
  .then((data) => {
    if(data.status === 200){
...
btn.addEventListener('click',()=>{
    ...
    axios.get(`http://bestshopnew.pythonanywhere.com/api/product/all?category=${category_value}&sortby=${sort_by_value}`)
    .then((data) => {
      if(data.status === 200){
...
const searchproduct = document.querySelector('#searchproduct'),
      ...
      axios.get(`http://bestshopnew.pythonanywhere.com/api/product/search?search=${searchproduct.value}`)
    .then((data) => {
      console.log(data)
      if(data.status === 200){
        productall = document.querySelectorAll('.product');
