...
speechbtn.addEventListener('click',(e)=>{
    ...
    switch(transcript){
     ...
     if (transcript.split(' ')[0] === "Jarvis" && transcript.split(' ')[1] === "show"  ){
        axios.get(`http://bestshopnew.pythonanywhere.com/api/product/all?category=${producttypr}`)
        .then((data) => {
          if(data.status === 200){
...
//     axios.get(`http://bestshopnew.pythonanywhere.com/api/product/all?category=${productType}`)
//     .then((data) => {
//       if(data.status === 200){
//         productall = document.querySelectorAll('.product');
//         for(let i=0;i<productall.length;i++){
